...
SELECT avg(unit_price) --<<- Update this line...
FROM products;
...
SELECT category_id
	   ,avg(unit_price) --<<- Update this line...
FROM products
group by category_id	               --<<- Update this line...
ORDER BY category_id;
...
SELECT category_id
		,product_name
		,unit_price
		,avg(unit_price) over() --<<- Update this line...
FROM products
ORDER BY category_id;
...
SELECT category_id
		,product_name
		,unit_price
		,avg(unit_price) over (partition by category_id) --<<- Update this line...
FROM products
ORDER BY category_id;
...
SELECT product_name 
       ,unit_price 
       ,category_id
       ,rank() over (order by unit_price desc) --<<- Update this line...
FROM products
order by category_id;
...
SELECT product_name 
       ,unit_price 
       ,category_id
       ,dense_rank() over (order by unit_price desc) --<<- Update this line...
FROM products
order by category_id;
...
SELECT product_name 
       ...
       ,rank () over (partition by category_id
       				order by unit_price desc)--<<- Update this line...---
FROM products;
...
SELECT product_name
       ...
       ,MAX(unit_price) OVER (PARTITION BY category_id
                              ROWS BETWEEN 1 PRECEDING AND 1 FOLLOWING) AS max_in_three_window_in_category --<<- Update this line...
FROM products;
...
SELECT product_name 
	   ,unit_price
       ,lag(unit_price) over () as previous_value --<<- Update this line...
FROM products;
...
SELECT product_name 
	   ...
	   ,((unit_price - lag(unit_price) over ()) / lag(unit_price) over ()) * 100 as percent_difference --- ???
FROM products;
